...
" 질병 관련 문서" 로드
'''''''''''''''''''''''''''''''''''''''''''''''''''
path = r'C:\Users\gram\SK_AIcamp_proj3\aihub_data\Training\원천데이터\TS_말뭉치데이터_내과'
...
hos_path = r"C:\Users\gram\SK_AIcamp_proj3\hospital_data\서울시_동물병원_인허가_정보.json"
...
vet_vectorstore = Chroma.from_documents(
    ...
    collection_name="vet_hospitals",  # 의료용이랑 겹치지 않게 다른 이름으로
)
...
def vet_hospital_node(state: CGRAState) -> dict:
    ...
    # 2) LLM이 보기 좋게 컨텍스트 문자열로 합치기
    context = "\n\n---\n\n".join([doc.page_content for doc in hospital_docs])

    # 3) 병원 정보 전용 프롬프트로 답변 생성
...
다음은 검색된 병원 목록(Context)입니다:

{context}

위 병원 목록 중에서, 사용자의 요구에 가장 잘 맞는 병원들을 최대 3개까지 골라
...
workflow.add_conditional_edges(
    "route",
    decide_route,   # state["route"] 값을 보고 "medical"/"vet_hospital"/"general" 리턴하는 함수
    {
        "medical": "retrieve",
