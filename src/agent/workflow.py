...
def validate_contextual_query(user_input: str, llm_model) -> bool:
    """
    맥락적 질문 유효성 검사 함수
    
    LLM의 추론 능력을 활용하여 질문의 전체적인 맥락과 의도가
    반려동물의 건강, 증상 진단, 질병 정보, 동물병원 추천과 관련되는지 판단합니다.
    
    Args:
        user_input: 사용자 질문
        llm_model: LLM 인스턴스 (ChatOpenAI 등)
        
    Returns:
        bool: 유효(True) / 무효(False)
    """
    
    # System Prompt: LLM에게 검증자 역할 부여
    validation_prompt = f"""당신은 질문의 유효성을 판단하는 전문 검증자(Validator)입니다.

## 역할
사용자의 질문이 **반려동물 의학/건강/병원 관련 명확한 의도**를 담고 있는지 판단하세요.

## 판단 기준 (다음 중 하나라도 해당하면 유효)
1. **반려동물의 증상 설명 또는 질병 관련 정보 요청**
   - 예: "강아지가 구토해요", "고양이 눈이 충혈됐어요", "반려견 피부병 증상은?"
   
2. **응급 상황 판단 및 조언 요청**
   - 예: "강아지가 경련을 일으켜요", "고양이가 숨을 헐떡거려요"
   
3. **동물병원 위치나 추천 관련 질문**
   - 예: "근처 동물병원 알려줘", "강남구 애견병원 추천"
   
4. **수의학적 지식에 대한 질문**
   - 예: "강아지 예방접종 시기는?", "고양이 중성화 수술이란?"

## 무효 판단 기준 (다음에 해당하면 무효)
- 반려동물과 무관한 주제: 날씨, 음식, 여행, 프로그래밍, 일반 상식 등
- 단순 인사나 잡담: "안녕", "심심해", "놀아줘" 등
- 반려동물은 언급되지만 건강/의료와 무관: "강아지 훈련 방법", "고양이 장난감 추천" 등

## 사용자 질문
"{user_input}"

## 출력 형식 (반드시 준수)
위 질문을 분석한 후, **오직 다음 두 단어 중 하나만** 출력하세요:
- 질문이 반려동물 의학/건강/병원과 관련되면: True
- 질문이 위 주제와 무관하면: False

**중요**: 설명이나 추가 텍스트 없이 "True" 또는 "False"만 출력하세요."""

    try:
        # LLM 호출
        response = llm_model.invoke([HumanMessage(content=validation_prompt)])
        result = response.content.strip()
        
        # 출력 파싱 (대소문자 무시, 여분 공백 제거)
        ...
        if "true" in result_lower:
            ...
            # LLM이 형식을 따르지 않은 경우, 보수적으로 False 반환
            print(f"[경고] LLM 출력 형식 오류: {result}")
            return False
            
    except Exception as e:
        print(f"[오류] 유효성 검사 중 예외 발생: {str(e)}")
        # 예외 발생 시 보수적으로 False 반환
        return False


def validate_question_node(state: AgentState) -> AgentState:
    """
    Node 0: 질문 유효성 검사 (LLM 기반 맥락 이해)
    
    LLM을 활용하여 질문의 맥락과 의도를 분석하고,
    반려동물 건강/의료와 관련된 질문인지 판단합니다.
    """
    print("\n[Node 0] 질문 유효성 검사 중 (맥락 분석)...")
    
    # Streamlit 진행 상황 표시
    global _streamlit_container
    if _streamlit_container:
        _streamlit_container.info("🔍 질문 유효성 검사 중... (AI 맥락 분석)")
    
    user_query = state["user_query"]
    
    # LLM 인스턴스 생성 (빠른 판단을 위해 temperature=0)
    llm = ChatOpenAI(
        model="gpt-4o-mini",
        temperature=0.0,
        max_tokens=10  # True/False만 출력하므로 짧게 설정
    )
    
    # 맥락적 유효성 검사 수행
    is_valid = validate_contextual_query(user_query, llm)
    
    if not is_valid:
        # 무효한 질문 -> 거절 응답 생성 후 즉시 종료
        rejection_message = """죄송합니다. 저는 **반려동물의 건강 증상 분석 및 응급도 판단**만을 전문으로 하는 AI 상담 시스템입니다.

입력하신 질문은 반려동물의 건강, 질병, 또는 동물병원과 관련이 없는 것으로 판단되어 답변을 드릴 수 없습니다.

## 💡 도움을 드릴 수 있는 질문 예시
• **증상 분석**: "강아지가 구토를 계속해요", "고양이 눈이 충혈됐어요"
• **응급 상황**: "반려견이 경련을 일으켜요", "고양이가 숨을 헐떡거려요"
• **병원 추천**: "서울 강남구 동물병원 알려주세요", "근처 24시간 응급 동물병원"
• **수의학 정보**: "강아지 예방접종 시기", "고양이 중성화 수술 정보"

반려동물의 건강에 대해 궁금하신 점이 있으시면 다시 질문해주세요."""
        ...
        if _streamlit_container:
            _streamlit_container.warning("⚠️ 반려동물 건강과 무관한 질문입니다.")
        
        print(f"[Node 0 완료] 무효한 질문 감지 (맥락 불일치) -> 워크플로우 종료")
        return state
    ...
    if _streamlit_container:
        _streamlit_container.success("✅ 유효한 질문입니다. (AI 맥락 분석 완료)")
    
    print(f"[Node 0 완료] 유효한 질문 확인 (맥락 일치) -> 증상 분석 진행")
    return state
...
def create_pet_health_agent() -> StateGraph:
    ...
    # Node 추가
    workflow.add_node("validate_question", validate_question_node)  # 질문 유효성 검사 노드 추가
