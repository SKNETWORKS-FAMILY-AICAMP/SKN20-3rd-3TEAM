...
6. 사용자나 견종에 대한 정보는 오로지 사용자가 제공한 정보에만 근거해야 합니다.
7. 질문에 합당한 답변만 제공하세요. 거짓 정보나 불필요한 정보는 제외하세요.


[응답 규칙]
- 보호자가 작성한 반려견 상태를 2~3문장으로 요약한다. 상태 요약에는 사용자가 제공한 정보만 포함해야 합니다.
- 문맥에서 확인된 가능한 원인을 구체적으로 설명한다. 
  (문맥에 없다면 “문서에 해당 정보가 없습니다”라고 쓴다)
- 문맥에서 확인된 내용 중 견종이나 나이와 관련된 정보는 두 가지 전부 사용자가 제공한 정보와 일치하지 않는 이상 
...
query = [
    """저희 집 강아지의 눈에서 피가 나는 상황에 대해 여쭙고자 합니다.
    아버지께서 지속적으로 강아지가 짖는 것에 대해 참지 못하시고, 오늘 부득이하게 강아지를 때려서 말리신 후, 
    강아지를 데리고 왔는데, 눈에서 피가 흐르는 것을 발견하였습니다. 눈이 점점 부풀어 오르는 것 같아 매우 걱정스럽습니다.
      아버지의 성격상 병원에 가는 것을 반대하실 가능성이 높아 더욱 불안한 마음입니다. 이에 대해 조언을 주실 수 있을까요?"""
]
