...
def load_json_files(data_dir: str) -> List[Dict[str, Any]]:
    ...
    logger.info(f"JSON 파일 로드 시작: {data_dir}")
    ...
    for json_file in data_path.rglob("*.json"):
        try:
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                logger.debug(f"로드: {json_file.name}")
        except Exception as e:
            logger.error(f"JSON 파일 로드 실패 {json_file}: {str(e)}")
            continue
    
    logger.info(f"JSON 파일 로드 완료: {len(json_files)}개")
    return json_files
            ...
            if isinstance(value, str):
                ...
                if value.endswith(('.pdf', '.md', '.txt')) and os.path.exists(value):
                    ...
                    try:
                        ...
                        logger.warning(f"파일 읽기 실패 {value}: {str(e)}")
...
def create_documents_from_json(json_files: List[Dict[str, Any]]) -> List[Document]:
    ...
    logger.info(f"Document 생성 시작: {len(json_files)}개 JSON 파일")
